...
const Portfolio = () => {
  ...
  const leftIndex =
    activeIndex === 0 ? filteredItems.length - 1 : activeIndex - 1;
  const centerIndex = activeIndex;
  const rightIndex =
    activeIndex === filteredItems.length - 1 ? 0 : activeIndex + 1;

  const handleLeftClick = () => {
    const newIndex =
      activeIndex - 1 >= 0 ? activeIndex - 1 : filteredItems.length - 1;
    setActiveIndex(newIndex);
  };

  const handleRightClick = () => {
    const newIndex =
      activeIndex + 1 < filteredItems.length ? activeIndex + 1 : 0;
    setActiveIndex(newIndex);
  };
    ...
    <section id="portfolio" className="app__portfolio">
      ...
      <div className="app__portfolio-interface">
        <div className="app__portfolio-categories">
          {portfolioCategories.map((category, index) => (
            <div
              ...
              className={`app__portfolio-category ${
        ...
        <div className="app__portfolio-items">
          <div
            className="app__portfolio-items-button-left"
            onClick={handleLeftClick}
          >
            <FaArrowLeft />
          </div>
          <div
            className="app__portfolio-items-button-right"
          ...
          <div className="app__portfolio-items-center">
            <iframe
              src={filteredItems[centerIndex].streamableLink}
              frameborder="0"
              width="100%"
              height="100%"
              allowfullscreen
